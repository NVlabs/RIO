...
class StandardROIHeads(ROIHeads):
    ...
    def forward(
        ...
        if self.training:
            ...
            # pred_instances, bbox_passed_idx, all_cls_scores = self._forward_box(
            # features, proposals
            # )
            # During inference cascaded prediction is used: the mask and keypoints heads are only
            # applied to the top scoring box detections.
            pred_instances = self.forward_with_given_boxes(features, pred_instances)
            # pred_instances[0].all_cls_scores = all_cls_scores[0]
            # pdb.set_trace()
    ...
    def _forward_box(
        ...
        if self.training:
            ...
            # pdb.set_trace()
            # return (pred_instances, bbox_passed_idx, all_cls_scores)
            return pred_instances, bbox_passed_idx
