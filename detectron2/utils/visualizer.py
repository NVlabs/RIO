...
class Visualizer:
    ...
    def __init__(
        ...
        self._default_font_size = max(
            np.sqrt(self.output.height * self.output.width) // 65, 20 // scale
        )
    ...
    def draw_instance_predictions(self, predictions, colors=None):
        """
        ...
        target_classes = [
            int(x.rstrip()) for x in open("lvis0.5_rare_common30_cats.txt", "r")
        ]
        ...
        if colors is None:
            if self._instance_mode == ColorMode.SEGMENTATION and self.metadata.get(
                ...
                colors = ["b" if c in target_classes else "g" for c in classes]
                alpha = 0.8
            else:
                colors = None
                alpha = 0.5
    ...
    def draw_box(self, box_coord, alpha=0.5, edge_color="g", line_style="-"):
        ...
        linewidth = max(self._default_font_size / 4, 2)
